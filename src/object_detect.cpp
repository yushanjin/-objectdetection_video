...
Result ObjectDetect::Postprocess(cv::Mat& frame, aclmdlDataset* modelOutput) {
    ...
    for (uint ImgIndex = 0; ImgIndex < kOutputTensorSize; ImgIndex ++) {
        ...
        uint32_t dataSize = 0;
        float* detectData = (float *)GetInferenceOutputItem(dataSize, modelOutput, (kOutputTensorSize - ImgIndex - 1));
    ...
    for (uint32_t i = 0; i < result.size(); i++) {
        ...
        oneResult.lt.x = result[i].lt_x * widthScale;
        oneResult.lt.y = result[i].lt_y * heightScale;
        oneResult.rb.x = result[i].rb_x * widthScale;
        oneResult.rb.y = result[i].rb_y * heightScale;
        oneResult.result_text = yolov3Label[result[i].attribute] + ":" + std::to_string(score) + "\%";
